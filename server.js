...
const app = express();
// Використовуємо порт, який видає Railway, або 3000 для тесту
const PORT = process.env.PORT || 3000;
const MONGO_URI = process.env.MONGODB_URI;

console.log("------------------------------------------------");
console.log("🦈 STARTING BARRACUDA SYSTEM...");
console.log("------------------------------------------------");

// Підключення до БД (не блокує запуск сервера)
if (!MONGO_URI) {
    console.warn("⚠️ УВАГА: Змінна MONGODB_URI не знайдена. Сайт працюватиме без бази даних.");
} else {
...
// Налаштування статики - важливо для завантаження CSS/JS
...
const ApplicationSchema = new mongoose.Schema({
    ...
    status: { type: String, default: 'pending' }, submittedBy: String, adminComment: String,
    createdAt: { type: Date, default: Date.now }
});
...
// Інші API ендпоінти
app.get('/api/users', async (req, res) => { try { const u = await User.find().sort({ regDate: -1 }); res.json(u); } catch(e) { res.json([]); } });
app.delete('/api/users/:username', async (req, res) => { try { await User.findOneAndDelete({ username: req.params.username }); res.json({ success: true }); } catch (e) { res.status(500).json({ success: false }); } });
app.put('/api/users/:username/role', async (req, res) => { try { await User.findOneAndUpdate({ username: req.params.username }, { role: req.body.role }); res.json({ success: true }); } catch(e) { res.status(500).json({ success: false }); } });
...
app.get('/api/applications', async (req, res) => { const a = await Application.find().sort({ createdAt: -1 }); res.json(a.map(x => ({...x._doc, id: x._id}))); });
app.get('/api/applications/my', async (req, res) => { const a = await Application.find().sort({ createdAt: -1 }); res.json(a.map(x => ({...x._doc, id: x._id}))); });
app.put('/api/applications/:id', async (req, res) => { try { await Application.findByIdAndUpdate(req.params.id, req.body); res.json({ success: true }); } catch(e) { res.status(500).json({ success: false }); } });
app.delete('/api/applications/:id', async (req, res) => { await Application.findByIdAndDelete(req.params.id); res.json({ success: true }); });
...
app.get('/api/tickets', async (req, res) => { const t = await Ticket.find().sort({ createdAt: -1 }); res.json(t.map(x => ({...x._doc, id: x._id}))); });
app.put('/api/tickets/:id', async (req, res) => { 
    try { 
        const { message, status } = req.body; 
        const update = {}; 
        if (status) update.status = status; 
        if (message) update.$push = { messages: message }; 
        await Ticket.findByIdAndUpdate(req.params.id, update); 
        res.json({ success: true }); 
    } catch(e) { res.status(500).json({ success: false }); } 
});

// Цей маршрут має бути останнім
app.get("*", (req, res) => { 
    const indexPublic = path.join(__dirname, "public", "index.html");
    res.sendFile(indexPublic, (err) => {
